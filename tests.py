...
class TestCrunchbaseMembers(unittest.TestCase):
    ...
    def testLoadDataNotBulkData(self):
        ...
        self.assertFalse(members.loadData())

    def testLoadDataBuldData(self):
        testcsvfilecontents = """
...
"""
    ...
    def testLookupCrunchbaseKeyEnv(self):
        with unittest.mock.patch.object(os, 'getenv', return_value="gotit") as mock_method:
            members = CrunchbaseMembers(bulkdata = False)
            self.assertEqual(members.crunchbaseKey,"gotit")

    def testSetCrunchbaseKeyInit(self):
        members = CrunchbaseMembers(crunchbaseKey = "gotit")
        self.assertEqual(members.crunchbaseKey,"")

        members = CrunchbaseMembers(crunchbaseKey = "gotit",bulkdata = False)
        self.assertEqual(members.crunchbaseKey,"gotit")
